...
graph TD
    subgraph Alpha[opengisch/presentations]
        ...
        C --> D(Build all directories using revealmd-js and copy the build under a 'web' subdirectory)
    ...
    subgraph Beta [opengisch/presentations_deployed]
        E --> |on push| F(update root 'index.html' so that all and only the files on the production branch are listed)
        F --> G(add files to opengisch/presentations_deployed:production)
    end
